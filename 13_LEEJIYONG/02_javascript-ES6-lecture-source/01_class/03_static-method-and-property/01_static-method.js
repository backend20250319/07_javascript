// 정적 메소드
// 특정 클래스 인스턴스가 아니고 클래스 '전체'에 필요한 기능을 만들 때 사용한다.
class Student {
  ...
  // 클래스 선언부 안에 위치하고 static 키워드를 붙이면 정적메소드가 선언된다.
...
Student.staticMethod = function () {
  console.log(
    `staticMethod는 메소드를 프로퍼티 형태로 직접 할당하는 것과 동일하다.`
  );
};
...
class User {
  ...
  // 클래스에 정적 메소드를 만들어 팩토리 메소드를 구현할 수 있다.
