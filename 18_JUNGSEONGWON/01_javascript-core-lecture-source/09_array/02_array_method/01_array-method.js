...
const arr = [];
// 배열의 생서자 함수는 Array
console.log(arr.const);
// 배열의 프로토타입 객체는 Array.prototype
console.log(Object.getPrototypeOf(arr) === Array.prototype);

// Array.pototype.indexof
...
console.log(`chieseFood.pop(): ${chinesefood.pop()}`);
console.log(`chieseFood.pop(): ${chinesefood.pop()}`);
console.log(`chieseFood.pop(): ${chinesefood.pop()}`);
...
// concat 이후 새로운 주소값을 생성

const mix = idol1.concat(idol2);
const mix2 = idol3.concat(idol1, idol2);

console.log(`idol1 기준 으로 idol2 배열을 concat : ${mix}`);
console.log(`idol3 기준 으로 idol1,idol2 배열을 concat : ${mix2}`);
console.log(`-----> `, idol1 === mix); // mix 이후 idol1과 동일한 지 확인 결과는 : false
...
// slice (시작 인덱스, 종료 인덱스) 원본은 건들이지 않는다.
console.log(`front.slice(1,3) : ${front.slice(1, 3)}`);
console.log(`front.slice(1,3) : ${front.slice(1)}`);
console.log(`front.slice(1,3) : ${front}`);

// splice (index, 제거 수 , 추가값1, 추가값2,....)
console.log(`front.splice(3,1,"React") : ${front.splice(3, 1, "React")}`);
console.log(`fornt : ${front}`);
