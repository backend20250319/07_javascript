...
// - GMT(그리니치 평군시) : UTC와 초의 소수점 단위에서만 차이가 나기 때문에 일상에서는 혼용되어 사용한다. - KST(한국 표준시) :
// UTC에 9시간을 더한 시간이다.

// Date 객체는 내부적으로 날짜와 시간을 나타내는 정수 값을 가진다.
// 1970년 1월 1일 00:00:00(UTC)를 기점으로 Date 객체가 나타내는 날짜와 시간까지의 밀리초를
// 나타낸다.
// 1. new Date() : 현재 날짜와 시간을 가지는 Date 객체 반환
console.log(new Date());
console.log(`====================================`);
// 2. new Date(milliseconds) : 1970년 1월 1일 00:00:00(UTC)를 기점으로 인수로 전달 된 밀리초만큼 경과한 날짜 와 시간을 나타내는 Date 객체 반환
...
console.log(`====================================`);
// Date.UTC : 1970년 1월 1일 00:00:00(UTC)을 기점으로 인수로 전달 된 지정 시간까지의 밀리 초를 숫자로 반환한다.
// 인수는 로컬 타임(KST)이 아닌 UTC로 인식된다.
console.log(Date.UTC(1970, 0, 1)); // 0
console.log(`====================================`);
// Date.parse :1970년 1월 1일 00:00:00(UTC)을 기점으로 인수로 전달된 지정 시간(new Date(dateString)의 인수와 
// 동일한 형식까지의 밀리초를 숫자로 반환한다. ### Date.UTC
...
date.setTime(5 * 24 * 60 * 60 * 1000); // 5일을 더해주는 내용으로 1970-01-06~ 식으로 나온다. 
