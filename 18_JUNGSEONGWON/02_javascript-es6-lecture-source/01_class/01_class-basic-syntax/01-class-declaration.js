...
Teacher.prototype.introduce = function () {
  console.log(`안녕하세요 저는 ${this.group}교사 ${this.name}입니다.`);
}
...
// 1. 클래스 생성자를 new 와 함께 호출하지 않으면 에러가 발생하도록 함
// Student();
Teacher(); 

// 2. 클래스에 정의된 메소드는 열거가 불가능하다.
for (method in student) {
  console.log(`반복문 student : ${method}`);
} // introduce가 출력되지 않는다 즉 클래스에서 정의된 메소드가 출력되지 않는다.
console.log(`==========================================`);
